...
export const Drawer = ({ id, children }: Props): ReactElement => {
  ...
  return (
    <>
      <div
        className={`${openMinicart && "translate-x-[100vw]"} fixed w-full bg-gray-700 opacity-80 top-0 right-0 h-screen p-4 overflow-y-auto z-39`}
        onClick={() => toggleMiniCart()}
      ></div>
      <div
        id={`drawer-${id}`}
        className={`bg-white fixed ${openMinicart && "translate-x-[100vw]"} top-0 right-0 z-40 h-screen p-4 overflow-y-auto  transition-transform  bg-neutral-primary-soft w-96 border-e border-default`}
        tabIndex={-1}
